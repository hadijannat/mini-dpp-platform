...
class TestGS1DigitalLink:
    ...
    def test_gs1_link_format(self):
        ...
        url = self.service.build_gs1_digital_link("01234567890128", "SN001")
        assert url == "https://id.gs1.org/01/01234567890128/21/SN001"

    def test_gtin_padded_to_14_digits(self):
        """Short GTIN is zero-padded on the left to 14 digits."""
        url = self.service.build_gs1_digital_link("12348", "SN001")
        assert "/01/00000000012348/" in url

    def test_gtin_non_digits_stripped(self):
        """Non-digit characters are stripped from the GTIN."""
        url = self.service.build_gs1_digital_link("123-456-784", "SN001")
        # Only digits remain; the cleaned value is 123456784 -> padded to 14
        assert "/01/00000123456784/" in url

    def test_serial_special_chars_encoded(self):
        """Special characters in the serial number are URL-encoded."""
        url = self.service.build_gs1_digital_link("01234567890128", "SN/2024 #01")
        # "/" -> %2F, " " -> %20, "#" -> %23
    ...
    def test_custom_resolver_url(self):
        ...
        url = self.service.build_gs1_digital_link(
            "01234567890128", "SN001", resolver_url="https://resolver.example.com"
        )
...
class TestAssetIdExtraction:
    ...
    def test_explicit_gtin_field(self):
        ...
        gtin, _, is_pseudo = self.service.extract_gtin_from_asset_ids(
    ...
    def test_pseudo_gtin_deterministic(self):
        ...
        gtin_a, _, _ = self.service.extract_gtin_from_asset_ids(ids)
        gtin_b, _, _ = self.service.extract_gtin_from_asset_ids(ids)
        assert gtin_a == gtin_b

    def test_pseudo_gtin_numeric_only(self):
        """Pseudo-GTIN consists of digits only."""
        gtin, _, _ = self.service.extract_gtin_from_asset_ids(
    ...
    def test_serial_from_asset_ids(self):
        ...
        _, serial, _ = self.service.extract_gtin_from_asset_ids(
    ...
    def test_missing_serial_returns_empty(self):
        ...
        _, serial, _ = self.service.extract_gtin_from_asset_ids({"gtin": "01234567890123"})
        assert serial == ""
