...
export function SourcesTab() {
  ...
  const [deleteSourceState, setDeleteSource] = useState<OPCUASourceResponse | null>(null);
  ...
  function handleDelete() {
    if (!deleteSourceState) return;
    deleteMutation.mutate(deleteSourceState.id, {
  ...
  return (
    <div className="space-y-4">
      ...
      <ConfirmDialog
        open={!!deleteSourceState}
        onOpenChange={(open) => {
          if (!open) setDeleteSource(null);
        }}
        title="Delete OPC UA Source"
        description={`This will permanently remove "${deleteSourceState?.name ?? ''}" and all associated configuration. This action cannot be undone.`}
        confirmLabel="Delete"
        variant="destructive"
